...
import com.tripndroid.tdsettings.fragments.Processor;
import com.tripndroid.tdsettings.fragments.MemoryManagement;
import com.tripndroid.tdsettings.fragments.IOScheduler;
...
public class BootReceiver extends BroadcastReceiver {
    ...
    private void configureCPU(Context ctx) {
        ...
        String availableFrequenciesLine = Helpers.readOneLine(Processor.FREQ_LIST_FILE);
        String availableGovernorsLine = Helpers.readOneLine(Processor.GOV_LIST_FILE);
        ...
        if (noSettings) {
            ...
            if (maxFrequency != null && frequencies != null && frequencies.contains(maxFrequency)) {
                Helpers.writeOneLine(Processor.FREQ_MAX_FILE, maxFrequency);
            }
            if (minFrequency != null && frequencies != null && frequencies.contains(minFrequency)) {
                Helpers.writeOneLine(Processor.FREQ_MIN_FILE, minFrequency);
            }
            if (governor != null && governors != null && governors.contains(governor)) {
                Helpers.writeOneLine(Processor.GOV_FILE, governor);
            }
    ...
    private void configureIOSched(Context ctx) {
        ...
        String availableIOSchedulersLine = Helpers.readOneLine(IOScheduler.IOSCHED_LIST_FILE);
        ...
        if (noSettings) {
            ...
            if (ioscheduler != null && ioschedulers != null && ioschedulers.contains(ioscheduler)) {
                Helpers.writeOneLine(IOScheduler.IOSCHED_LIST_FILE, ioscheduler);
            }
